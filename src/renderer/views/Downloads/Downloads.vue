    ...
    <FtCard class="card">
      <FtFlexBox class="header-row">
        <h2>
          ...
          <span v-if="queue.length > 0" class="badge">
      ...
      <div v-if="queue.length === 0" class="empty-state">
        <div class="empty-icon-container">
          <FontAwesomeIcon :icon="['fas', 'download']" class="empty-icon" />
        </div>
        <p class="empty-text">Your download queue is empty.</p>
        <p class="empty-subtext">Start downloading videos or audio to see them here.</p>
      </div>
      ...
      <div v-else class="queue-list">
        ...
        >
          <div class="card-left">
            <div class="status-icon-wrapper" :class="item.status">
              <FontAwesomeIcon
          ...
          <div class="card-center">
            <div class="item-title-row">
              <router-link :to="`/watch/${item.videoId}`" class="item-title" :title="item.title">
                {{ item.title }}
              </router-link>
              ...
              <FtFlexBox class="progress-details">
                <div class="progress-left">
                  <span class="status-label" :class="item.status">
                    ...
                    <template v-if="item.status === 'downloading'">Downloading...</template>
                    <template v-if="item.status === 'completed'">Completed</template>
                    <template v-if="item.status === 'error'">Failed</template>
                  </span>
                  <span v-if="item.status === 'downloading'" class="stats-divider">&middot;</span>
                  <span v-if="item.status === 'downloading'" class="download-speed">{{ item.speed }}</span>
                  <span v-if="item.status === 'downloading'" class="stats-divider">&middot;</span>
                  <span v-if="item.status === 'downloading'" class="download-eta">{{ t('Settings.Download Settings.ETA') }}: {{ item.eta }}</span>
                </div>
            ...
            <div v-if="item.status === 'error'" class="error-msg" :title="item.errorMsg">
              {{ item.errorMsg }}
            </div>
